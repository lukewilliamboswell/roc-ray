...
A hosted effect the host admits during the commit phase --
`enforcePhase(name, during_commit)` in `src/host_native.zig` -- needs a
`Program.Action` that reaches it, because `update` is pure and `render!` only
draws, so an action is the whole of what a running app can change about host
state. Add the variant, its `run_action!` arm, and a receiver-form constructor
on the owning type; none of that crosses the ABI. If the effect deliberately has
no action, write down why. Either way the name goes in
[`docs/action-coverage.md`](docs/action-coverage.md), and `zig build lint` fails
until it does.
