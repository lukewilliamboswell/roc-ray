...
fn get_roc_app_object(target: &RocRaySupportedTarget) -> AppType {
    match target {
        ...
        RocRaySupportedTarget::Linux => {
            ...
            let app_o = manifest_dir().join("app.o");
            let lib_app_o = manifest_dir().join("libapp.o");

            if app_o.exists() {
                AppType::Static(app_o.to_path_buf())
            } else if lib_app_o.exists() {
                let cwd_libapp_so = manifest_dir().join("libapp.so");

                let output = std::process::Command::new("ld")
                    .args(&["-shared", "-o", &cwd_libapp_so.to_string_lossy(), "libapp.o"])
                    .output()
