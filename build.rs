...
fn main() {
    ...
    // Check if the app.o file exists in the root directory
    if !Path::new("app.obj").exists() {
    ...
    let lib_app_path = Path::new(&out_dir).join("app.lib");
    let output = std::process::Command::new("lib")
        .args(&[
            format!("/OUT:{}", lib_app_path.to_str().unwrap()),
            "app.obj",
        ])
        .output()
    ...
    if !output.status.success() {
        panic!("lib command failed with status: {}", output.status);
    }
