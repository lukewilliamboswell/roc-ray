//! Code tidiness checks for the roc-ray codebase.
//!
//! Adapted from TigerBeetle/Roc's tidy.zig with modifications for roc-ray.
...
pub fn main(init: std.process.Init) !void {
    ...
    var dead_files_detector = DeadFilesDetector.init();
...
fn tidyFile(
    ...
    if (std.mem.startsWith(u8, file.path, "vendor/")) return;
    if (std.mem.startsWith(u8, file.path, "platform/")) return;
    if (std.mem.startsWith(u8, file.path, "examples/")) return;
    if (std.mem.startsWith(u8, file.path, "zig-out/")) return;
    if (std.mem.startsWith(u8, file.path, ".zig-cache/")) return;
    // Generated by `roc glue` — not hand-maintained, so don't lint it.
    if (std.mem.endsWith(u8, file.path, "src/roc_platform_abi.zig")) return;
...
const DeadFilesDetector = struct {
    ...
    fn visit(detector: *DeadFilesDetector, gpa: Allocator, file: SourceFile) Allocator.Error!void {
        ...
        // Skip directories that shouldn't be tracked for dead file detection
        if (std.mem.startsWith(u8, file.path, "vendor/")) return;
        if (std.mem.startsWith(u8, file.path, "platform/")) return;
        if (std.mem.startsWith(u8, file.path, "examples/")) return;
        if (std.mem.startsWith(u8, file.path, "zig-out/")) return;
        ...
        // Only scan src/, ci/, and build files for imports
        const should_scan = is_src_file or
            std.mem.startsWith(u8, file.path, "ci/") or
            std.mem.eql(u8, file.path, "build.zig");
