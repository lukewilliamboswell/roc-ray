...
DoomDebug := [].{
	Hit : { linedef : U64, distance : F64, start : DoomMap.Vertex, end : DoomMap.Vertex, lines : List(Str) }

	trace : DoomMap.Map, DoomLevel.State, DoomSim.Vec2, DoomSim.Angle -> Try(Hit, [NoRayHit, OutsideMap])
	trace = |map, level, pos, angle| {
		...
		sector = match DoomLevel.sector_at(map, origin) {
			Ok(value) => value
		...
		dynamic = line_dynamic(raw, found.linedef, DoomLevel.dynamic_sectors(map))
		masked = List.len(List.keep_if(spans, |span| span.kind == Middle and span.flags.two_sided))
		portal_text = match DoomLevel.portal(map, level, found.linedef, sector) {
...
side_description = |raw, level, side_ref, label|
	match side_ref {
		...
		Ok(index) => {
			...
			heights = DoomLevel.heights_for(level, side.sector) ?? crash "debug sector state missing"
