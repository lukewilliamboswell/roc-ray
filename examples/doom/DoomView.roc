...
## partitions. Movement bob itself is owned by DoomSim.View and is consumed by
...
expect {
	started = DoomView.advance(DoomView.initial, 1, { damaged: Bool.True, picked_up: Bool.True })
	whole = DoomView.advance(started, 7, { damaged: Bool.False, picked_up: Bool.False })
	part = DoomView.advance(DoomView.advance(started, 3, { damaged: Bool.False, picked_up: Bool.False }), 4, { damaged: Bool.False, picked_up: Bool.False })
	whole.damage == part.damage and whole.pickup == part.pickup and whole.damage == 5 and whole.pickup == 1
}

expect {
	flashes = DoomView.advance({ damage: 2, pickup: 3 }, 8, { damaged: Bool.False, picked_up: Bool.False })
