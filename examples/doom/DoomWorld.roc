...
DoomWorld := [].{
	...
	Actor : { id : U64, kind : ThingKind, pos : DoomSim.Vec2, angle : DoomSim.Angle, spawn_pos : DoomSim.Vec2, spawn_angle : DoomSim.Angle, health : I64, state : ActorState, ambush : Bool, reaction_time : I64, target_threshold : I64, move_count : I64, move_dir : I64, chase_frame : U64, just_attacked : Bool, dead_tics : U64 }
	OccupiedActor : { pos : DoomSim.Vec2, radius : F32 }
	ActorFacts : { player_pos : DoomSim.Vec2, has_sight : Bool, heard_sound : Bool, blockers : List(DoomSim.Segment), occupied : List(OccupiedActor), nightmare : Bool, used_door : Bool }
			...
			match value.state.mode {
				...
				Chase => {
					if value.state.remaining > 1 {
						...
						if facts.used_door {
							count = DoomWorld.random(rng)
							active = DoomWorld.random(count.rng)
							...
							if decision.attack {
								...
								active = DoomWorld.random(walk.rng)
	...
	chase_candidate : Actor, DoomSim.Vec2 -> DoomSim.Vec2
	chase_candidate = |value, target| {
		direction = direction_vector(chase_direction_index(value.pos, target))
		DoomSim.add(value.pos, DoomSim.scale(direction, actor_speed(value.kind)))
	}
	ambush_flag = 8.U64
...
look_sees_player = |actor, facts| {
	if !(facts.has_sight) {
		...
		to_player = DoomSim.sub(facts.player_pos, actor.pos)
		in_front = DoomSim.dot(actor.angle.forward(), to_player) >= 0
		in_front or approximate_distance(actor.pos, facts.player_pos) <= 64
	}
}

wake_random = |kind, rng|
	if kind == ZombieMan or kind == ShotgunGuy or kind == Imp DoomWorld.random(rng) else { rng, byte: 0 }
...
select_chase_move = |actor, facts, rng| {
	...
	if diagonal_try.moved {
		...
		axis_roll = DoomWorld.random(diagonal_try.rng)
		...
		if axis_try.moved {
			...
			if old_try.moved {
				...
				scan_roll = DoomWorld.random(old_try.rng)
...
try_chase_direction = |actor, direction_index, facts| {
	if direction_index < 0 {
		...
		candidate = DoomSim.add(actor.pos, DoomSim.scale(direction, actor_speed(actor.kind)))
		radius = DoomWorld.actor_radius_for(actor.kind)
		min_distance = radius + DoomWorld.player_collision_radius
		wall_blocked = if DoomSim.any_penetration(actor.pos, radius, facts.blockers) {
			DoomSim.path_deepens_penetration(actor.pos, candidate, radius, facts.blockers)
		} else {
			DoomSim.any_collision(candidate, radius, facts.blockers)
		}
		blocked = wall_blocked
			or DoomSim.distance_squared(candidate, facts.player_pos) < min_distance * min_distance
			or List.any(facts.occupied, |other| DoomSim.distance_squared(candidate, other.pos) < (radius + other.radius) * (radius + other.radius))
		if blocked {
			{ actor, moved: Bool.False }
		} else {
...
actor_attack = |actor, player_pos, has_sight, rng| {
	...
	if actor.kind == Demon or actor.kind == Spectre {
		...
		for _ in List.repeat({}, 3) {
			spread_a = DoomWorld.random($rng)
			spread_b = DoomWorld.random(spread_a.rng)
			roll = DoomWorld.random(spread_b.rng)
...
enemy_hitscan_hits = |from, target, spread_a, spread_b| {
	direction = DoomSim.normalize(DoomSim.sub(target, from))
	perpendicular = { x: 0 - direction.y, y: direction.x }
	spread_turns = I64.to_f32(U8.to_i64(spread_a) - U8.to_i64(spread_b)) / 4096
	shot_direction = DoomSim.normalize(DoomSim.add(direction, DoomSim.scale(perpendicular, spread_turns * 6.2831855)))
	to_target = DoomSim.sub(target, from)
	along = DoomSim.dot(to_target, shot_direction)
	across = F32.abs(to_target.x * shot_direction.y - to_target.y * shot_direction.x)
	along > 0 and across <= DoomWorld.player_collision_radius
}
...
expect {
	first = DoomWorld.random(DoomWorld.Rng.seed(0))
	second = DoomWorld.random(first.rng)
	third = DoomWorld.random(second.rng)
	wrapped = DoomWorld.random(DoomWorld.Rng.seed(255))
...
expect {
	...
	base = DoomWorld.player({ x: 0, y: 0 }, DoomSim.Angle.from_turns(0))
	clip = DoomWorld.collect(base, { id: 0, kind: DroppedClip, pos: base.sim.state.pos, taken: Bool.False })
	shotgun = DoomWorld.collect(base, { id: 1, kind: DroppedShotgun, pos: base.sim.state.pos, taken: Bool.False })
	clip.player.ammo.bullets == base.ammo.bullets + 5
		and shotgun.player.ammo.shells == 4
		and DoomWorld.owns(shotgun.player, Shotgun)
}
...
expect {
	...
	base = DoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False)
	far_behind : DoomWorld.ActorFacts
	...
	far = DoomWorld.tick_actor_with(base, far_behind, DoomWorld.Rng.seed(0))
	close = DoomWorld.tick_actor_with(base, close_behind, DoomWorld.Rng.seed(0))
...
expect {
	...
	zombie = DoomWorld.tick_actor_with(DoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False), facts, DoomWorld.Rng.seed(0))
	demon = DoomWorld.tick_actor_with(DoomWorld.actor(2, Demon, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False), facts, DoomWorld.Rng.seed(0))
...
expect {
	...
	blocked = DoomWorld.tick_actor_with(ready, { player_pos: { x: 100, y: 0 }, has_sight: Bool.False, heard_sound: Bool.False, blockers: [wall], occupied: [], nightmare: Bool.False, used_door: Bool.False }, DoomWorld.Rng.seed(0))
	close = DoomWorld.tick_actor_with(ready, { player_pos: { x: 32, y: 0 }, has_sight: Bool.False, heard_sound: Bool.False, blockers: [], occupied: [], nightmare: Bool.False, used_door: Bool.False }, DoomWorld.Rng.seed(0))
	blocked.actor.pos.x <= base.pos.x
		and DoomSim.distance_squared(close.actor.pos, { x: 32, y: 0 }) >= (DoomWorld.actor_radius + DoomWorld.player_collision_radius) * (DoomWorld.actor_radius + DoomWorld.player_collision_radius)
}
...
expect {
	...
	base = DoomWorld.actor(1, Imp, { x: 10, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False)
	wall : DoomSim.Segment
...
expect {
	...
	turn = DoomWorld.tick_actor_with(ready, facts, DoomWorld.Rng.seed(0))
...
expect {
	...
	direct = select_chase_move(base, facts, DoomWorld.Rng.seed(0))
	no_turnaround = select_chase_move({ ..base, move_dir: 5 }, facts, DoomWorld.Rng.seed(0))
...
expect {
	...
	selected = select_chase_move(base, facts, DoomWorld.Rng.seed(0))
...
expect {
	...
	base = DoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False)
	var $turn = { actor: { ..base, state: DoomWorld.state(Attack) }, rng: DoomWorld.Rng.seed(2), player_damage: 0, attack_kind: NoAttack }
	facts : DoomWorld.ActorFacts
	facts = { player_pos: { x: 128, y: 0 }, has_sight: Bool.True, heard_sound: Bool.False, blockers: [], occupied: [], nightmare: Bool.False, used_door: Bool.False }
...
expect {
	...
	facts : DoomWorld.ActorFacts
	facts = { player_pos: { x: 128, y: 0 }, has_sight: Bool.True, heard_sound: Bool.False, blockers: [], occupied: [], nightmare: Bool.False, used_door: Bool.False }
	miss = DoomWorld.tick_actor_with(ready, facts, DoomWorld.Rng.seed(0))
	hit = DoomWorld.tick_actor_with(ready, facts, DoomWorld.Rng.seed(2))
...
expect {
	...
	shot = DoomWorld.tick_actor_with(ready, facts, DoomWorld.Rng.seed(2))
...
expect {
	...
	normal_turn = DoomWorld.tick_actor_with(ready, ordinary, DoomWorld.Rng.seed(0))
	fast_turn = DoomWorld.tick_actor_with(ready, fast, DoomWorld.Rng.seed(0))
...
expect {
	...
	decision = attack_decision(ready, { x: 4096, y: 0 }, Bool.True, Bool.False, DoomWorld.Rng.seed(3))
	decision.attack and decision.rng.index() == 4
}

expect {
	DoomWorld.state_for(ZombieMan, Attack).remaining == 26
		and DoomWorld.state_for(ShotgunGuy, Attack).remaining == 30
		and DoomWorld.state_for(Imp, Attack).remaining == 22
		and DoomWorld.state_for(Demon, Attack).remaining == 24
		and DoomWorld.state_for(Spectre, Attack).remaining == 24
}
...
expect {
	...
	base = DoomWorld.actor(1, Imp, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False)
	killed = DoomWorld.damage_actor_random(base, 1000, DoomWorld.Rng.seed(1))
	killed.actor.state.remaining == DoomWorld.state_for(Imp, Dead).remaining - 1 and killed.rng.index() == 2
}

expect {
	barrel = DoomWorld.actor(1, Barrel, { x: 0, y: 0 }, DoomSim.Angle.from_turns(0), Bool.False)
	damaged = DoomWorld.damage_actor_random(barrel, 1, DoomWorld.Rng.seed(0))
...
expect {
	...
	inside = DoomWorld.tick_actor_with(ready, facts, DoomWorld.Rng.seed(0))
	boundary = DoomWorld.tick_actor_with(ready, { ..facts, player_pos: { x: 60, y: 0 } }, DoomWorld.Rng.seed(0))
...
expect {
	DoomWorld.actor_radius_for(ZombieMan) == 20
		and DoomWorld.actor_radius_for(ShotgunGuy) == 20
		and DoomWorld.actor_radius_for(Imp) == 20
		and DoomWorld.actor_radius_for(Demon) == 30
		and DoomWorld.actor_radius_for(Spectre) == 30
		and DoomWorld.actor_radius_for(Barrel) == 10
}
...
expect {
	spawned = DoomWorld.spawn(DoomMap.e1m1.raw().things, Medium)
	List.any(spawned.actors, |value| value.state.remaining != DoomWorld.state_for(value.kind, Look).remaining)
}
