...
segment_gen : Fuzz.Generator(DoomSim.Segment)
...
test = |input| {
	check_angle(input)
	check_sqrt(input)
	state0 = { ..DoomSim.initial(input.pos, DoomSim.Angle.from_turns(input.angle_turns)), momentum: input.momentum }
...
check_angle = |input| {
	t = bits_to_f32(input.turn_bits)
	{
		...
		added = DoomSim.Angle.from_turns(0.5).add(t).turns()
...
# ---------------------------------------------------------------------------
# P6: DoomSim.sqrt agrees with F32.sqrt on the squared-length domain.
...
check_sqrt = |input| {
	...
	if in_domain {
		...
		if !(F32.is_finite(got)) or rel > 0.0001 {
			crash "PROPERTY: sqrt_accuracy: DoomSim.sqrt(${Str.inspect(v)}) = ${Str.inspect(got)}, F32.sqrt = ${Str.inspect(want)}, rel_err = ${Str.inspect(rel)}"
		}
	}
}
...
check_state : Str, DoomSim.State -> {}
check_state = |label, s| {
	if !(F32.is_finite(s.pos.x) and F32.is_finite(s.pos.y)) {
		crash "PROPERTY: ${label}: non-finite pos ${Str.inspect(s.pos)}"
	}
	if !(F32.is_finite(s.momentum.x) and F32.is_finite(s.momentum.y)) or F32.abs(s.momentum.x) > DoomSim.max_move or F32.abs(s.momentum.y) > DoomSim.max_move {
	...
	view = s.view
	if !(F32.is_finite(view.bob)) or view.bob < 0 or view.bob > DoomSim.max_bob {
...
check_tic : DoomSim.State, DoomSim.Command, List(DoomSim.Segment) -> {}
...
check_neutral : DoomSim.State, List(DoomSim.Segment) -> {}
check_neutral = |state0, blockers| {
	resting = { ..state0, momentum: DoomSim.zero }
	s1 = DoomSim.tic(resting, DoomSim.neutral, blockers)
	if s1.pos != resting.pos or s1.momentum != DoomSim.zero {
...
segments_cross : Vec2, Vec2, DoomSim.Segment -> Bool
...
path_penetrates : Vec2, Vec2, DoomSim.Segment -> Bool
path_penetrates = |p0, p1, s| {
	...
	while $i < 64 {
		...
		if DoomSim.distance_to_segment_squared(p, s) < (radius - graze_depth) * (radius - graze_depth) {
			$hit = Bool.True
		}
		$i = $i + 1
	}
...
check_advance_result : Str, DoomSim.Advance -> {}
check_advance_result = |label, result| {
	rem = result.clock.remainder
	if !(F32.is_finite(rem)) or rem < 0 or rem >= DoomSim.tic_seconds {
		crash "PROPERTY: ${label}: remainder out of range ${Str.inspect(rem)}"
	}
	if result.tics > DoomSim.max_catch_up_tics {
...
check_advance_sequence : DoomSim.State, DoomSim.Command, List(DoomSim.Segment), List(U64) -> {}
check_advance_sequence = |state0, command, blockers, elapsed_seq| {
	var $clock = DoomSim.clock(state0)
	var $total_tics = 0.U64
	var $total_elapsed = 0.0.F64
	for raw in elapsed_seq {
		elapsed = U64.to_f32(raw) / 35000
		result = DoomSim.advance($clock, elapsed, command, blockers)
...
check_wild_elapsed = |state0, command, blockers, bits| {
	elapsed = bits_to_f32(bits)
	{
		...
		if F32.is_finite(elapsed) and elapsed >= 1 and !(result.dropped and result.tics == DoomSim.max_catch_up_tics) {
...
check_partition : DoomSim.State, DoomSim.Command, List(DoomSim.Segment), U64 -> {}
check_partition = |state0, command, blockers, k| {
	whole = DoomSim.advance(DoomSim.clock(state0), DoomSim.tic_seconds, command, blockers)
	...
	part = DoomSim.tic_seconds / U64.to_f32(k)
	var $clock = DoomSim.clock(state0)
	var $tics = 0.U64
	var $i = 0.U64
	while $i < k {
		result = DoomSim.advance($clock, part, command, blockers)
	...
	if $tics == 1 {
		...
		if F32.abs($clock.remainder - DoomSim.tic_seconds) > 0.00001 {
