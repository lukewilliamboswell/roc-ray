...
expected_tics = |parts| {
	...
	var $cum = 0
	var $prev = 0
	for _ in List.repeat({}, List.len(DoomTrace.commands)) {
		for k in parts {
			$cum = $cum + k
			tic = $cum / 64
			if tic > $prev {
...
golden : DoomTrace.Run
golden = DoomTrace.run([1.0001])
...
test = |input| {
	...
	if total <= 66 and !(has_epsilon) {
		if DoomTrace.checksum(run.trace) != DoomTrace.golden_checksum or run.trace != golden.trace {
	...
	if List.len(parts) > 1 and !(has_epsilon) {
		reference = DoomTrace.run([U64.to_f32(total) / 64])
...
first_divergence : List(DoomTrace.Snapshot), List(DoomTrace.Snapshot), U64 -> Str
...
expect {
	...
	List.all(cases, |fractions| DoomTrace.checksum(DoomTrace.run(fractions).trace) == DoomTrace.golden_checksum)
}
