...
```sh
cd examples/roc-doom-e1m1
roc build --fuzz fuzz_sim.roc && ./fuzz_sim run --time=120
```

| Target | Module(s) | Driver | Campaign |
|---|---|---|---|
| `fuzz_sim.roc` | RocDoomSim | libFuzzer | ~1080 s, 5.7 M execs, cov 355 |
| `fuzz_map.roc`, `fuzz_map_decode.roc` | RocDoomMap validate/derive, RocDoomLevel queries over generated maps | libFuzzer | ~1230 s / 715 k execs (map); 150 s / 1.76 M execs (decode) |
| `fuzz_level.roc` | RocDoomLevel movers over E1M1 | `roc test` seeded streams (see T1) | 400 seeds × 40 commands, 394 s |
| `fuzz_world.roc`, `fuzz_trace.roc` | RocDoomWorld inventory/combat; RocDoomTrace partitions | libFuzzer | 300 s / 3.0 M execs (world); 2 × 300 s / ~3.7 k execs (trace) |
...
- Severity: hang
- **Status: FIXED** — `wrap_turn` reduces via `to_i64_wrap` in one step, treats non-finite and |v| ≥ 2^24 as zero turns; regression expect in `RocDoomSim.roc`; `fuzz_sim` 90 s / 4.6 M execs clean with `guard_angle_hang` off.
- Location: `RocDoomSim.roc:272` (`wrap_turn`), via `Angle.from_turns` (:14) and `Angle.add` (:20)
...
- Root cause: `wrap_turn` recurses by ±1. Above 2^24 the F32 ulp is ≥ 2 so
  ...
  `RocDoomLevel.sector_at` fails. `Command.turn` feeds this directly, so a bad
...
### S2. `RocDoomSim.sqrt` is inaccurate outside roughly [1e-7, 1e7]
- Severity: wrong-result (fidelity)
- **Status: FIXED** — `sqrt` delegates to the builtin `F32.sqrt` (non-positive input stays 0); regression expect in `RocDoomSim.roc`; `fuzz_sim` 60 s / 3.2 M execs clean with `guard_sqrt` off.
- Location: `RocDoomSim.roc:233-244` (`sqrt`); consumers `length` (:222), `normalize` (:226)
...
- Root cause: 14 fixed Newton iterations seeded at `value`; early iterations
  ...
  `RocDoomSprites.roc:54,82`, `RocDoomPresentation.roc:107`, `RocDoomRuntime.roc:127`
...
- **Status: FIXED** — `move_with_slide` picks the earliest-hit blocker and rejects a slide whose *path* deepens penetration of any blocker (`path_deepens_penetration`); regression expect from both fuzz repros in `RocDoomSim.roc`; `fuzz_sim` 120 s / 3.0 M execs clean with `guard_graze` and `guard_multi_blocker` off.
- Location: `RocDoomSim.roc:141-160` (`move_with_slide`), the
...
## RocDoomMap validation and RocDoomLevel spatial queries (generated maps)

Core property: **anything `RocDoomMap.validate` accepts is safe to query.**

### M1. Cyclic BSP node graphs pass validation and hang `RocDoomLevel.sector_at`
- Severity: hang
- **Status: FIXED** — `validate_child` requires node children to precede their parent (`NodeChildNotEarlier`); regression expect in `RocDoomMap.roc`; `fuzz_map` now asserts validation rejects cyclic graphs.
- Location: `RocDoomMap.roc:208-223` (`validate_nodes`/`validate_child`); hang in `RocDoomLevel.roc:193` (`descend`)
...
- Severity: crash
- **Status: FIXED** — `lowest_adjacent_ceiling/floor` return `Try`; sectors with no adjacent sector are skipped and a tagged special that moves nothing answers `NotUsable`; regression expect in `RocDoomLevel.roc`.
- Location: `RocDoomLevel.roc:466` (`lowest_adjacent_ceiling`: `crash "door sector has no adjacent sector"`)
...
- Suggested fix: return `NotUsable` (vanilla: the sector simply does not move)
  instead of crashing, or validate tag → geometry in `RocDoomMap.validate`.

### M3. Coordinate-equal vertices yield zero-length blocking/collision segments
- Severity: domain-gap
- **Status: FIXED** — `validate_linedefs`/`validate_segs` compare coordinates (`same_position`); regression expect in `RocDoomMap.roc`; `fuzz_map` now asserts validation rejects them.
- Location: `RocDoomMap.roc:147` (`validate_linedefs` compares vertex *indices*); same in `validate_segs` (:187)
...
- Root cause: the degenerate check is by index, so a zero-length segment
  reaches `blocking_segments`/`collision_segments`. `RocDoomSim.distance_to_segment_squared`
...
polygons rejected by strict `cross > 0`. `RocDoomMap.decode` on 1.76 M arbitrary
and JSON-spliced strings: always `Ok`/`Err`, never crash or hang.

---

## RocDoomLevel movers over E1M1

### L1. Special 62 is implemented as a permanent door; on E1M1 its tags name the lift sectors
- Severity: fidelity + wrong-result
- **Status: FIXED** — `use_line` routes 62 to `activate_tagged_lifts`; a fresh door also clamps `open` to at least its current ceiling; regression expect in `RocDoomLevel.roc`; `fuzz_level` 100 seeds clean with `guard_special_62` off.
- Location: `RocDoomLevel.roc:151` (`62 => activate_tagged_doors(...)`) and the doc comment at :139-141
...
- **Status: FIXED** — `activate_door` reverses an existing door (Closing→Opening, Opening/Waiting→Closing) keeping its heights; tagged activation skips sectors with an active door (vanilla `EV_DoDoor`); regression expect in `RocDoomLevel.roc`; `fuzz_level` 100 seeds clean with `guard_door_reuse` off.
- Location: `RocDoomLevel.roc:458-464` (`activate_door`)
...
## RocDoomWorld inventory and combat

### W1. Stimpack / Medikit / Berserk *lower* health above 100 back to 100
- Severity: wrong-result
- **Status: FIXED** — `give_health` refuses at or above its cap, Berserk uses `max(health, 100)`; regression expect in `RocDoomWorld.roc`; `fuzz_world` 90 s / 1.0 M execs clean with `guard_b2` off.
- Location: `RocDoomWorld.roc:533-534, 556` via `give_health` at :615
...
- **Status: FIXED** — `acquire_weapon` takes the ammo result and reports `!already_owned or ammo_given`; regression expect in `RocDoomWorld.roc`; `fuzz_world` 90 s / 1.0 M execs clean with weapons checked against `vanilla_accepts`.
- Location: `RocDoomWorld.roc:568-579` (`acquire_weapon`), all five weapon kinds
...
- Location: `RocDoomWorld.roc:535, 555` via `give_health`
...
- Confidence: high on vanilla; medium on whether it is wanted (no recorded decision).
...
- **Status: FIXED** — early return for `amount <= 0`; regression expect in `RocDoomWorld.roc`; the previously disabled `fuzz_world` check is now live (60 s / 714 k execs clean).
- Location: `RocDoomWorld.roc:334-338`
...
## RocDoomTrace partition invariance (characterisation, not a bug)

- Location: `RocDoomSim.roc:82-96` (`advance`) as driven by `RocDoomTrace.roc:71-84` (`run_parts`)
- **Exact sums of 1.0 do not under-tick.** `[1.0]`, `[0.25 × 4]`, `[0.5, 0.5]`,
  `[1/64, 63/64]`, `[1/3 × 3]`, `[0.1 × 10]`, `[0.7, 0.3]`, … all give 24 tics
  and the golden checksum. The `1.0001` epsilon in `RocDoomTrace.roc`'s expects
...
- Standalone repro (no Doom code): a record `{ heights : List(I64), floors : List(Item), tic : U64 }`,
  ...
  is correct in both forms. Every `RocDoomLevel` state-returning function uses the
...
- Also seen from the map target: `RocDoomLevel.dynamic_sectors` returned
...
- `List.map2(a, b, |x, y| { x, y })` over `RocDoomWorld.Actor` records segfaults `roc build --fuzz` (`roc check` passes). Worked around with an index loop.
- `roc build --fuzz` never finished (5+ min, 100 % CPU) when a helper took `raw : RocDoomMap.Raw` and called `List.get(raw.linedefs, line)`; `roc check` fine. Worked around by passing `List(U64)`.
- `--opt=size --specialize=no` crashes the compiler; `--opt=dev`/`interpreter` are rejected for `--fuzz`.
- F32 fields and the opaque `RocDoomSim.Angle`/`RocDoomWorld.Rng` block derived `==` on `Player`/`ActorTic` ("type does not support equality"), yet `RocDoomTrace.Snapshot` (which has F32 fields) compares fine — the failure seems specific to records containing the opaque types.
