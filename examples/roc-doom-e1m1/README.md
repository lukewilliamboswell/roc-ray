...
RocDOOM is a Doom-compatible engine written in Roc. This example is a
complete E1M1 vertical slice built from the pinned Freedoom 0.13.0 Phase 1
WAD. It is not affiliated with or endorsed by id Software or the Freedoom
project; see [`THIRD-PARTY-LICENCES.md`](../../THIRD-PARTY-LICENCES.md).

While developing RocRay, run it against the local platform source from the
repository root:
...
`A`/`D` or Left/Right to strafe, mouse movement to turn, left mouse or Space to fire, and `E` to use doors and
switches. Number keys `2`, `3`, `4`, `5`, `6`, and `8` select owned weapons.

The host samples input once per rendered cycle, but the simulation runs at a
fixed 35 Hz, so one cycle yields zero, one or several tics. Mouse deltas and key
presses are therefore accumulated in `RocDoomControls.Pending` and handed to the
first tic a cycle produces, never to each catch-up tic. Held axes (`forward`,
`side`, `fire`) are sampled fresh per cycle and do repeat per tic, matching
Doom's own ticcmd.
...
`RocDoomReplay.roc` contains a frozen Baby-skill ordinary-command trace;
...
shorter `RocDoomTrace.roc` test proves host-cycle partition invariance without
...
```sh
scripts/all_tests.py --only roc-doom-e1m1 --skip-platform-build
```
...
```sh
ROC=~/roc_nightly-linux_x86_64-2026-09-10-a670e34/roc
...
3. Run `roc test examples/roc-doom-e1m1/main.roc` with the same compiler, then sample the
...
Asset provenance, licensing, and the behavioural-reference policy are
recorded in [`THIRD-PARTY-LICENCES.md`](../../THIRD-PARTY-LICENCES.md).
No commercial Doom data is used or required.
