...
parse_world : Str -> RocDoomAssets.WorldAtlas
parse_world = |text|
...
parse_sprites : Str -> RocDoomAssets.SpriteAtlas
parse_sprites = |text|
	match Json.parse(text) {
		Ok(atlas) => atlas
...
expect {
	wall = RocDoomAssets.texture("AQCOMP01")
	floor = RocDoomAssets.flat("AQF001")
	List.len(RocDoomAssets.world.entries) == 158
		and match wall {
...
expect {
	front = RocDoomAssets.sprite("POSS", "A", 1)
	back = RocDoomAssets.sprite("POSS", "A", 5)
	missing = RocDoomAssets.sprite("NOPE", "A", 1)
	match front {
