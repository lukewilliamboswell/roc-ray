...
RocDoomLevel := [].{
	...
	sector_at : RocDoomMap.Map, Point -> Try(U64, [OutsideMap])
	sector_at = |map, point| {
		raw = map.raw()
	...
	light_for : RocDoomMap.Map, State, U64 -> Try(I64, [SectorOutOfRange(U64)])
	...
	portal : RocDoomMap.Map, State, U64, U64 -> Try(Portal, [LinedefOutOfRange(U64), NotTwoSided, SectorNotOnLine(U64)])
	portal = |map, state, linedef, from_sector| {
		...
		blocked = RocDoomMap.line_flags(line.flags).blocks_players
	...
	collision_segments : RocDoomMap.Map, State, U64 -> List(CollisionSegment)
	...
	global_collision_segments : RocDoomMap.Map, State -> List(CollisionSegment)
		...
		for line in raw.linedefs {
			blocks = match line.left_sidedef {
				...
				Ok(left_index) => {
					...
					RocDoomMap.line_flags(line.flags).blocks_players or floor_delta > 24 or opening < 56
				}
			}
			if blocks {
	...
	crossed_lines : RocDoomMap.Map, Point, Point -> List(U64)
	...
	dynamic_sectors : RocDoomMap.Map -> List(U64)
	dynamic_sectors = |map| {
		raw = map.raw()
	...
	## previous batches.
	render_changed : RocDoomMap.Map, State, State -> Bool
	...
	## requests level exit.
	use_line : RocDoomMap.Map, State, U64, Keys -> UseResult
	...
	monster_use_line : RocDoomMap.Map, State, U64 -> UseResult
	monster_use_line = |map, state, linedef| {
		raw = map.raw()
		match List.get(raw.linedefs, linedef) {
			Ok(line) if line.special == 1 and !(RocDoomMap.line_flags(line.flags).secret) => monster_activate_local_door(raw, state, line)
			_ => NotUsable
		}
	}
	...
	cross_line : RocDoomMap.Map, State, U64 -> UseResult
...
render_changed_in = |map, before, after, sectors, index|
	match List.get(sectors, index) {
		Err(_) => Bool.False
		Ok(sector) => {
			before_heights = RocDoomLevel.heights_for(before, sector) ?? crash "dynamic sector missing"
			after_heights = RocDoomLevel.heights_for(after, sector) ?? crash "dynamic sector missing"
			before_light = RocDoomLevel.light_for(map, before, sector) ?? crash "dynamic light missing"
			after_light = RocDoomLevel.light_for(map, after, sector) ?? crash "dynamic light missing"
...
collision_segments_indexed = |map, raw, state, sector, lines, offset|
	match List.get(lines, offset) {
		...
		Ok(index) => {
			...
			blocks = match line.left_sidedef {
				Err(Null) => Bool.True
				Ok(_) => match RocDoomLevel.portal(map, state, index, sector) {
...
advance_tics = |state, count|
	if count == 0 state else advance_tics(RocDoomLevel.tick(state), count - 1)

expect {
	map = RocDoomMap.e1m1
	start = map.player_start() ?? crash "E1M1 player start missing"
	sector = RocDoomLevel.sector_at(map, { x: I64.to_f64(start.position.x), y: I64.to_f64(start.position.y) }) ?? crash "player outside BSP"
	heights = RocDoomLevel.heights_for(RocDoomLevel.initial(map), sector) ?? crash "sector missing"
	heights.ceiling - heights.floor >= 56
}

expect {
	map = RocDoomMap.e1m1
	state = RocDoomLevel.initial(map)
	var $equivalent = Bool.True
	var $sum = 0.U64
	var $maximum = 0.U64
	for indexed in List.map_with_index(map.raw().sectors, |_sector, index| index) {
		indexed_segments = RocDoomLevel.collision_segments(map, state, indexed)
		...
		count = RocDoomLevel.collision_candidate_count(state, indexed)
		$sum = $sum + count
		...
		match (entry.line.right_sidedef, entry.line.left_sidedef) {
			(Ok(right_index), Ok(left_index)) => {
				...
				for from_sector in [right_sector, left_sector] {
					portal = RocDoomLevel.portal(map, state, entry.index, from_sector) ?? crash "validated portal missing"
					from = RocDoomLevel.heights_for(state, portal.from_sector) ?? crash "from sector missing"
					to = RocDoomLevel.heights_for(state, portal.to_sector) ?? crash "to sector missing"
					expected_bottom = I64.max(from.floor, to.floor)
					expected_top = I64.min(from.ceiling, to.ceiling)
					expected_step = to.floor - from.floor
					expected_traversable = !(RocDoomMap.line_flags(entry.line.flags).blocks_players) and expected_step <= 24 and expected_top - expected_bottom >= 56
...
expect {
	...
	for _ in List.repeat({}, 80) {
		$opened = RocDoomLevel.tick($opened)
	}
	List.all(
		List.map_with_index(map.raw().sectors, |_sector, index| index),
		|sector| RocDoomLevel.collision_segments(map, $opened, sector) == collision_segments_from(map, map.raw(), $opened, sector, 0),
	)
}
...
expect {
	...
	map = RocDoomMap.e1m1
	initial = RocDoomLevel.initial(map)
	opened = RocDoomLevel.monster_use_line(map, initial, 55)
	match opened {
		Activated(opening) => {
			repeated = RocDoomLevel.monster_use_line(map, opening, 55)
			closing = RocDoomLevel.use_line(map, opening, 55, { blue: Bool.False, yellow: Bool.False, red: Bool.False })
			match closing {
				Activated(value) => match RocDoomLevel.monster_use_line(map, value, 55) {
...
expect {
	...
	for _ in List.repeat({}, 64) {
		$after64 = RocDoomLevel.tick($after64)
	}
	flash_bright = RocDoomLevel.light_for(map, initial, 32) ?? -1
	flash_dark = RocDoomLevel.light_for(map, $after64, 32) ?? -1
	strobe_initial = RocDoomLevel.light_for(map, initial, 90) ?? -1
	strobe_dark = RocDoomLevel.light_for(map, RocDoomLevel.tick(initial), 90) ?? -1
	var $after16 = initial
	for _ in List.repeat({}, 16) {
		$after16 = RocDoomLevel.tick($after16)
	}
	strobe_bright = RocDoomLevel.light_for(map, $after16, 90) ?? -1
...
expect {
	map = RocDoomMap.e1m1
	state = RocDoomLevel.initial(map)
	locked = RocDoomLevel.use_line(map, state, 421, { blue: Bool.False, yellow: Bool.False, red: Bool.False })
	unlocked = RocDoomLevel.use_line(map, state, 421, { blue: Bool.True, yellow: Bool.False, red: Bool.False })
...
expect {
	map = RocDoomMap.e1m1
	state = RocDoomLevel.initial(map)
	activated = RocDoomLevel.use_line(map, state, 55, { blue: Bool.False, yellow: Bool.False, red: Bool.False })
	match activated {
		Activated(opening) => {
			door = List.get(opening.doors, 0) ?? crash "door missing"
			before = RocDoomLevel.heights_for(opening, door.sector) ?? crash "door sector missing"
			after = RocDoomLevel.heights_for(RocDoomLevel.tick(opening), door.sector) ?? crash "door sector missing"
...
expect {
	...
	match RocDoomLevel.cross_line(map, initial, 593) {
		Activated(moving) => {
			...
			before = RocDoomLevel.heights_for(moving, lift.sector) ?? crash "lift sector missing"
			after = RocDoomLevel.heights_for(RocDoomLevel.tick(moving), lift.sector) ?? crash "lift sector missing"
			finished = advance_tics(moving, 400)
			finish_height = RocDoomLevel.heights_for(finished, lift.sector) ?? crash "lift sector missing"
...
expect {
	...
	crossed = RocDoomLevel.crossed_lines(map, { x: mid_x + dy, y: mid_y - dx }, { x: mid_x - dy, y: mid_y + dx })
...
expect {
	map = RocDoomMap.e1m1
	initial = RocDoomLevel.initial(map)
	match RocDoomLevel.use_line(map, initial, 753, { blue: Bool.False, yellow: Bool.False, red: Bool.False }) {
		Activated(moving) => {
			mover = List.get(moving.floors, 0) ?? crash "tagged floor mover missing"
			before = RocDoomLevel.heights_for(moving, mover.sector) ?? crash "floor sector missing"
			after = RocDoomLevel.heights_for(RocDoomLevel.tick(moving), mover.sector) ?? crash "floor sector missing"
...
expect {
	...
	match activated {
		Activated(opening) => {
			...
			closed = RocDoomLevel.portal(map, initial, 55, from_sector) ?? crash "door portal missing"
			opened_state = advance_tics(opening, 80)
			opened = RocDoomLevel.portal(map, opened_state, 55, from_sector) ?? crash "door portal missing"
			closed_segments = RocDoomLevel.collision_segments(map, initial, from_sector)
			opened_segments = RocDoomLevel.collision_segments(map, opened_state, from_sector)
			closed_global = RocDoomLevel.global_collision_segments(map, initial)
			opened_global = RocDoomLevel.global_collision_segments(map, opened_state)
			!(closed.traversable)
...
expect {
	...
	initial = RocDoomLevel.initial(map)
	use = |state| match RocDoomLevel.use_line(map, state, 55, no_keys) {
		Activated(next) => next
		_ => crash "door 55 not usable"
	}
	first = use(initial)
	door = List.get(first.doors, 0) ?? crash "door missing"
	at_rest = RocDoomLevel.heights_for(initial, door.sector) ?? crash "sector missing"
	...
	settled_heights = RocDoomLevel.heights_for(settled, door.sector) ?? crash "sector missing"
	open_state = advance_tics(first, 100)
	open_heights = RocDoomLevel.heights_for(open_state, door.sector) ?? crash "sector missing"
	closing = advance_tics(use(open_state), 10)
	closing_heights = RocDoomLevel.heights_for(closing, door.sector) ?? crash "sector missing"
	closed_again = RocDoomLevel.heights_for(advance_tics(closing, 1000), door.sector) ?? crash "sector missing"
...
expect {
	...
	no_keys = { blue: Bool.False, yellow: Bool.False, red: Bool.False }
	initial = RocDoomLevel.initial(map)
	match RocDoomLevel.use_line(map, initial, 1064, no_keys) {
		Activated(started) => {
			lift = List.get(started.lifts, 0) ?? crash "lift missing"
			rest = RocDoomLevel.heights_for(initial, lift.sector) ?? crash "sector missing"
			lowered = RocDoomLevel.heights_for(advance_tics(started, 40), lift.sector) ?? crash "sector missing"
			settled = advance_tics(started, 400)
			raised = RocDoomLevel.heights_for(settled, lift.sector) ?? crash "sector missing"
...
expect {
	...
	map = RocDoomMap.validate(raw) ?? crash "orphan fixture should validate"
	state = RocDoomLevel.initial(map)
	no_keys = { blue: Bool.False, yellow: Bool.False, red: Bool.False }
	...
	unusable(RocDoomLevel.use_line(map, state, 0, no_keys))
		and unusable(RocDoomLevel.cross_line(map, state, 1))
			and unusable(RocDoomLevel.use_line(map, state, 2, no_keys))
				and unusable(RocDoomLevel.cross_line(map, state, 3))
}
