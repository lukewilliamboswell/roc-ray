...
RocDoomMap := [].{
	...
	Map :: { raw : Raw, blocking_segments : List(BlockingSegment) }.{
		...
		## Per-sector moving geometry remains the responsibility of RocDoomLevel.
...
derive_blocking_segments = |raw, index|
	match List.get(raw.linedefs, index) {
		Err(_) => []
		Ok(line) => {
			flags = RocDoomMap.line_flags(line.flags)
...
side_spans = |raw, line, line_index, side_name, own_ref, other_ref|
	match own_ref {
		...
		Ok(own_index) => {
			...
			common : SpanCommon
			common = { linedef: line_index, side: side_name, start, end, x_offset: own.x_offset, y_offset: own.y_offset, sector: own.sector, light_level: sector.light_level, flags: RocDoomMap.line_flags(line.flags), special: if line.special == 0 NoSpecial else Special({ number: line.special, tag: line.tag }) }
			match other_ref {
				Err(Null) => {
...
SpanCommon : { linedef : U64, side : RocDoomMap.Side, start : RocDoomMap.Vertex, end : RocDoomMap.Vertex, x_offset : I64, y_offset : I64, sector : U64, light_level : I64, flags : RocDoomMap.LineFlags, special : RocDoomMap.Special }

texture_span : Try(Str, [Null]), SpanCommon, RocDoomMap.WallKind, I64, I64, RocDoomMap.VerticalPeg -> List(RocDoomMap.WallSpan)
...
expect {
	...
	map = RocDoomMap.validate(raw) ?? crash "fixture should validate"
...
expect {
	plain = RocDoomMap.validate(fixture(fixture_line(0, Err(Null)), [fixture_side(0, Err(Null), Err(Null), Ok("STONE"))], [fixture_sector(-16, 112, 160)])) ?? crash "plain wall"
	unpegged = RocDoomMap.validate(fixture(fixture_line(0x0010, Err(Null)), [fixture_side(0, Err(Null), Err(Null), Ok("STONE"))], [fixture_sector(-16, 112, 160)])) ?? crash "unpegged wall"
...
expect {
	...
	map = RocDoomMap.validate(raw) ?? crash "unpegged portal should validate"
	spans = map.wall_spans()
...
expect {
	...
	match RocDoomMap.validate(bad_vertex) {
		Err(VertexOutOfRange(_)) => Bool.True
		_ => Bool.False
	}
		and match RocDoomMap.validate(bad_sector) {
			Err(SectorOutOfRange(_)) => Bool.True
			_ => Bool.False
		}
			and match RocDoomMap.validate(bad_light) {
...
expect {
	flags = RocDoomMap.line_flags(0x01ff)
...
expect {
	...
	match RocDoomMap.validate(missing_right) {
		Err(MissingRightSidedef(_)) => Bool.True
		_ => Bool.False
	}
		and match RocDoomMap.validate(bad_side) {
			Err(SidedefOutOfRange(_)) => Bool.True
			_ => Bool.False
		}
			and match RocDoomMap.validate(bad_two_sided) {
...
expect {
	...
	match RocDoomMap.validate(bad_format) {
		Err(InvalidFormat(_)) => Bool.True
		_ => Bool.False
	}
		and match RocDoomMap.validate(empty_name) {
			Err(EmptyMapName) => Bool.True
			_ => Bool.False
		}
			and match RocDoomMap.validate(degenerate) {
				Err(DegenerateLinedef(_)) => Bool.True
				_ => Bool.False
			}
				and match RocDoomMap.validate(bad_angle) {
...
expect {
	map = RocDoomMap.validate(surface_fixture) ?? crash "surface fixture should validate"
...
expect {
	...
	match RocDoomMap.validate(duplicate_polygon) {
		Err(DuplicateSubsectorPolygon(_)) => Bool.True
		_ => Bool.False
	}
		and match RocDoomMap.validate(bad_direction) {
			Err(InvalidSegDirection(_)) => Bool.True
			_ => Bool.False
		}
			and match RocDoomMap.validate(bad_child) {
...
expect {
	...
	rejected = |raw, expected| match RocDoomMap.validate(raw) {
		Err(NodeChildNotEarlier(detail)) => detail == expected
		_ => Bool.False
	}
	accepted = |raw| match RocDoomMap.validate(raw) {
		Ok(_) => Bool.True
		Err(_) => Bool.False
	}
...
expect {
	...
	seg_rejected = |raw, expected| match RocDoomMap.validate(raw) {
