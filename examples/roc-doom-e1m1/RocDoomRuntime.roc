...
RocDoomRuntime := [].{
	...
	Projectile : { id : U64, owner : U64, target : U64, pos : RocDoomSim.Vec2, momentum : RocDoomSim.Vec2, damage : I64, remaining : U64 }
	Aggro : { hunter : U64, target : U64 }
	Explosion : { pos : RocDoomSim.Vec2, remaining : U64 }
	WeaponState : { cooldown : U64, phase : U64 }
	World : { doom : RocDoomWorld.World, skill : RocDoomWorld.Skill, projectiles : List(Projectile), explosions : List(Explosion), sound_origins : List(RocDoomSim.Vec2), aggro : List(Aggro), next_projectile_id : U64, weapon : WeaponState, secrets_found : U64, visited_secret_sectors : List(U64), phase : Phase }
	Advance : { world : World, tics : U64, dropped : Bool, fired : Bool, projectile_saturated : Bool }
	MapAdvance : { world : World, level : RocDoomLevel.State, tics : U64, dropped : Bool, fired : Bool, projectile_saturated : Bool }

	initial : RocDoomWorld.World -> World
	initial = |doom| initial_for_skill(doom, Medium)

	initial_for_skill : RocDoomWorld.World, RocDoomWorld.Skill -> World
	...
	advance = |world, elapsed, command, blockers| {
		if world.phase != Playing {
			...
			sim = RocDoomSim.advance(world.doom.player.sim, elapsed, command, all_blockers)
	...
	advance_in_map : World, F32, RocDoomSim.Command, List(RocDoomSim.Segment), RocDoomMap.Map, RocDoomLevel.State -> MapAdvance
	advance_in_map = |world, elapsed, command, extra_blockers, map, level|
		advance_in_map_first(world, elapsed, command, command, extra_blockers, map, level)

	## Advance like `advance_in_map`, delivering `first` to the opening tic of
	## this call and `repeat` to every catch-up tic after it. The weapon and
	## firing loop below must select on the same index as `RocDoomSim`'s tic loop,
	## or a latched slot request would land on a different tic than the movement
	## it was sampled with. `advance_in_map` passes the same command twice.
	advance_in_map_first : World, F32, RocDoomSim.Command, RocDoomSim.Command, List(RocDoomSim.Segment), RocDoomMap.Map, RocDoomLevel.State -> MapAdvance
	advance_in_map_first = |world, elapsed, first, repeat, extra_blockers, map, level| {
		if world.phase != Playing {
			{ world, level, tics: 0, dropped: Bool.False, fired: Bool.False, projectile_saturated: Bool.False }
		} else {
			actor_blockers = actor_segments(world.doom.actors)
			sim = RocDoomSim.advance_first_with(
			...
			var $index = 0.U64
			for _ in List.repeat({}, sim.tics) {
				command = if $index == 0 first else repeat
				$index = $index + 1
				$next = prepare_weapon($next, command.weapon_slot, command.fire)
	...
	tic_hearing_in_map : World, List(U64), List(RocDoomSim.Segment), RocDoomMap.Map, RocDoomLevel.State -> { world : World, projectile_saturated : Bool, fired : Bool }
	...
	tic : World, Bool, List(RocDoomSim.Segment) -> { world : World, projectile_saturated : Bool, fired : Bool }
	...
	tic_hearing : World, List(U64), List(RocDoomSim.Segment) -> { world : World, projectile_saturated : Bool, fired : Bool }
	...
	activate_monster_doors = |world, map, level| {
		...
		for actor in world.doom.actors {
			...
			if ready {
				...
				candidate = RocDoomWorld.chase_candidate(actor, target_pos)
				match RocDoomLevel.sector_at(map, { x: F32.to_f64(actor.pos.x), y: F32.to_f64(actor.pos.y) }) {
					Err(_) => {}
					Ok(sector) => match List.find_first(
						RocDoomLevel.collision_segments(map, $level, sector),
						|entry| {
							usable = match List.get(map.raw().linedefs, entry.linedef) {
								Ok(line) => line.special == 1 and !(RocDoomMap.line_flags(line.flags).secret)
								Err(_) => Bool.False
							}
							usable and RocDoomSim.circle_hits_segment(candidate, RocDoomWorld.actor_radius_for(actor.kind), collision_to_segment(entry.start, entry.end))
						},
					) {
						Err(_) => {}
						Ok(entry) => match RocDoomLevel.monster_use_line(map, $level, entry.linedef) {
							Activated(next) => {
								$level = next
	...
	tic_hearing_with = |world, heard_actors, intercept_blockers, actor_blockers_for, door_users| {
		...
		for actor in world.doom.actors {
			...
			occupied = if actor.state.mode == Chase and actor.state.remaining <= 1 List.map(List.keep_if(world.doom.actors, |other| other.id != actor.id and other.state.mode != Dead), |other| { pos: other.pos, radius: RocDoomWorld.actor_radius_for(other.kind) }) else []
			target = aggro_target(world.aggro, actor.id, world.doom.actors)
			target_pos = match target {
				Ok(value) => value.pos
			...
			if turn.attack_kind == ProjectileAttack {
				if List.len($projectiles) < max_projectiles {
					direction = RocDoomSim.normalize(RocDoomSim.sub(target_pos, actor1.pos))
					target_id = match target {
						Ok(value) => value.id
						Err(PlayerTarget) => player_sound_id
					}
					$projectiles = List.append($projectiles, { id: $next_id, owner: actor1.id, target: target_id, pos: actor1.pos, momentum: RocDoomSim.scale(direction, projectile_speed_for_skill(world.skill)), damage: turn.player_damage, remaining: projectile_lifetime })
					$next_id = $next_id + 1
				} else {
					$saturated = Bool.True
				}
		...
		player0 = damage_player_for_skill(RocDoomWorld.tick_player_powers(world.doom.player), $damage + projectile_step.damage, world.skill)
	...
	line_of_sight : RocDoomSim.Vec2, RocDoomSim.Vec2, List(RocDoomSim.Segment) -> Bool
	...
	actor_turn_for_skill = |actor, facts, rng, skill| {
		first = RocDoomWorld.tick_actor_with(actor, facts, rng)
		...
		if skill != Nightmare or !(fast_state) {
			...
			second = RocDoomWorld.tick_actor_with(first.actor, facts, first.rng)
			{
				actor: second.actor,
				rng: second.rng,
	...
	blockers_for_player : RocDoomMap.Map, RocDoomLevel.State, RocDoomSim.Vec2 -> List(RocDoomSim.Segment)
	blockers_for_player = |map, level, pos| {
		permanent = List.map(map.blocking_segments(), |segment| to_segment(segment.start, segment.end))
		sector = RocDoomLevel.sector_at(map, { x: F32.to_f64(pos.x), y: F32.to_f64(pos.y) }) ?? return permanent
		dynamic = List.map(
			RocDoomLevel.collision_segments(map, level, sector),
			|segment| {
	...
	cross_specials : RocDoomMap.Map, RocDoomLevel.State, RocDoomSim.Vec2, RocDoomSim.Vec2 -> { level : RocDoomLevel.State, exited : Bool }
	cross_specials = |map, level, from, to| {
		var $level = level
		var $exited = Bool.False
		for line in RocDoomLevel.crossed_lines(map, { x: F32.to_f64(from.x), y: F32.to_f64(from.y) }, { x: F32.to_f64(to.x), y: F32.to_f64(to.y) }) {
			match RocDoomLevel.cross_line(map, $level, line) {
				Activated(next) => {
					$level = next
				}
				Exit => {
	...
	use_forward : RocDoomMap.Map, RocDoomLevel.State, RocDoomSim.Vec2, RocDoomSim.Angle, RocDoomWorld.Keys -> RocDoomLevel.UseResult
	use_forward = |map, level, pos, angle, keys|
		match usable_line_ahead(map, pos, angle) {
			Ok(index) => RocDoomLevel.use_line(map, level, index, keys)
	...
	usable_line_ahead : RocDoomMap.Map, RocDoomSim.Vec2, RocDoomSim.Angle -> Try(U64, [NoUsableLine])
	usable_line_ahead = |map, pos, angle| {
		raw = map.raw()
		end = RocDoomSim.add(pos, RocDoomSim.scale(angle.forward(), use_distance))
...
global_map_blockers = |map, level| {
	List.map(
		RocDoomLevel.global_collision_segments(map, level),
		|segment| {
			start: { x: F64.to_f32_wrap(segment.start.x), y: F64.to_f32_wrap(segment.start.y) },
			end: { x: F64.to_f32_wrap(segment.end.x), y: F64.to_f32_wrap(segment.end.y) },
		},
		...
		if actor.state.mode == Chase and actor.state.remaining <= 1 {
			match RocDoomLevel.sector_at(map, { x: F32.to_f64(actor.pos.x), y: F32.to_f64(actor.pos.y) }) {
				...
				Ok(sector) => if !(List.any($cache, |entry| entry.sector == sector)) {
					map_blockers = RocDoomRuntime.blockers_for_player(map, level, actor.pos)
...
actor_blockers_from_cache = |cache, map, level, pos, extra_blockers| {
	sector = RocDoomLevel.sector_at(map, { x: F32.to_f64(pos.x), y: F32.to_f64(pos.y) }) ?? return List.concat(List.map(map.blocking_segments(), |segment| to_segment(segment.start, segment.end)), extra_blockers)
	entry = List.find_first(cache, |candidate| candidate.sector == sector) ?? return List.concat(RocDoomRuntime.blockers_for_player(map, level, pos), extra_blockers)
	entry.blockers
}
...
distance_to_segment_squared = |point, segment| {
	along = RocDoomSim.sub(segment.end, segment.start)
	length = RocDoomSim.length_squared(along)
	amount = if length <= 0 0 else F32.max(0, F32.min(1, RocDoomSim.dot(RocDoomSim.sub(point, segment.start), along) / length))
	RocDoomSim.distance_squared(point, RocDoomSim.add(segment.start, RocDoomSim.scale(along, amount)))
}
...
collect_nearby = |player, pickups, skill| {
	...
	for pickup in pickups {
		if !(pickup.taken) and RocDoomSim.distance_squared(player.sim.state.pos, pickup.pos) <= RocDoomRuntime.player_pickup_radius * RocDoomRuntime.player_pickup_radius {
			result = RocDoomWorld.collect_for_skill($player, pickup, skill)
...
actor_segments = |actors| {
	...
	for actor in actors {
		if actor.state.mode != Dead {
			r = RocDoomWorld.actor_radius_for(actor.kind)
...
actor_overlaps = |actor, actors|
	actor.state.mode != Dead and List.any(
		actors,
		|other| {
			distance = RocDoomWorld.actor_radius_for(actor.kind) + RocDoomWorld.actor_radius_for(other.kind)
			other.id != actor.id and other.state.mode != Dead and RocDoomSim.distance_squared(actor.pos, other.pos) < distance * distance
		},
	)

actor_needs_sight : RocDoomWorld.Actor -> Bool
actor_needs_sight = |actor|
	actor.state.mode == Look
		or (actor.state.mode == Chase and actor.state.remaining <= 1)
			or (actor.state.mode == Attack and !(actor.state.attacked) and actor.state.remaining == RocDoomWorld.attack_action_remaining(actor.kind))
...
advance_projectiles = |projectiles, player_pos, actors, blockers, rng| {
	...
	for projectile in projectiles {
		candidate = RocDoomSim.add(projectile.pos, projectile.momentum)
		path : RocDoomSim.Segment
		path = { start: projectile.pos, end: candidate }
		hits_wall = List.any(blockers, |blocker| projectile_hits_blocker(path, blocker))
		hits_player = projectile.target == RocDoomRuntime.player_sound_id and RocDoomSim.distance_to_segment_squared(player_pos, path) <= (RocDoomSim.player_radius + projectile_radius) * (RocDoomSim.player_radius + projectile_radius)
		hit_actor = projectile_actor_hit(projectile, actors, path)
		actor_before_player = match hit_actor {
			Ok(actor) => hits_player and RocDoomSim.distance_squared(path.start, actor.pos) < RocDoomSim.distance_squared(path.start, player_pos)
			Err(_) => Bool.False
		}
...
projectile_hits_blocker : RocDoomSim.Segment, RocDoomSim.Segment -> Bool
projectile_hits_blocker = |path, blocker| {
	...
	if separated {
		...
		segments_cross(path.start, path.end, blocker.start, blocker.end)
			or RocDoomSim.distance_to_segment_squared(path.start, blocker) <= radius2
				or RocDoomSim.distance_to_segment_squared(path.end, blocker) <= radius2
					or RocDoomSim.distance_to_segment_squared(blocker.start, path) <= radius2
						or RocDoomSim.distance_to_segment_squared(blocker.end, path) <= radius2
	}
}

projectile_actor_hit : RocDoomRuntime.Projectile, List(RocDoomWorld.Actor), RocDoomSim.Segment -> Try(RocDoomWorld.Actor, [NoActorHit])
projectile_actor_hit = |projectile, actors, path| {
	var $hit = Err(NoActorHit)
	var $best = -1.F32
	for actor in actors {
		radius = RocDoomWorld.actor_radius_for(actor.kind) + projectile_radius
		if actor.id != projectile.owner and actor.state.mode != Dead and RocDoomSim.distance_to_segment_squared(actor.pos, path) <= radius * radius {
			distance = RocDoomSim.distance_squared(path.start, actor.pos)
	...
	for hit in hits {
		...
		for actor in $actors {
			if actor.id == hit.target and actor.state.mode != Dead {
				result = RocDoomWorld.damage_actor_random(actor, hit.damage, $rng)
				$rng = result.rng
				$retarget = actor.target_threshold == 0 and hit.source != actor.id and result.actor.state.mode != Dead
				retaliating = if $retarget and result.actor.state.mode == Look { ..result.actor, target_threshold: 100, state: RocDoomWorld.state_for(result.actor.kind, Chase) } else if $retarget { ..result.actor, target_threshold: 100 } else result.actor
...
resolve_deaths = |previous, world, blockers| {
	...
	while $pending_index < List.len($pending) {
		...
		if dead.kind == Barrel {
			if List.len($sounds) < RocDoomRuntime.max_sound_origins {
				$sounds = List.append($sounds, dead.pos)
			}
			var $damaged = []
			for actor in $actors {
				if actor.id != dead.id and actor.state.mode != Dead and RocDoomRuntime.line_of_sight(dead.pos, actor.pos, blockers) {
					damage = radius_damage(dead.pos, actor.pos, RocDoomWorld.actor_radius_for(actor.kind))
					if damage > 0 {
						result = RocDoomWorld.damage_actor_random(actor, damage, $rng)
						$rng = result.rng
			...
			$actors = $damaged
			if $player.health > 0 and RocDoomRuntime.line_of_sight(dead.pos, $player.sim.state.pos, blockers) {
				player_damage = radius_damage(dead.pos, $player.sim.state.pos, RocDoomWorld.player_collision_radius)
...
nightmare_respawns = |world, blockers| {
	...
	if world.doom.player.sim.state.tic % 32 != 0 {
		...
		var $next = []
		for actor in $actors {
			eligible = actor.state.mode == Dead and actor.kind != Barrel and actor.dead_tics >= 12 * RocDoomSim.tic_rate
			if eligible {
				roll = RocDoomWorld.random($rng)
				$rng = roll.rng
				radius = RocDoomWorld.actor_radius_for(actor.kind)
				occupied = RocDoomSim.distance_squared(actor.spawn_pos, world.doom.player.sim.state.pos) < (radius + RocDoomWorld.player_collision_radius) * (radius + RocDoomWorld.player_collision_radius)
					or List.any(
						$actors,
						|other| {
							minimum = radius + RocDoomWorld.actor_radius_for(other.kind)
							other.id != actor.id and other.state.mode != Dead and RocDoomSim.distance_squared(actor.spawn_pos, other.pos) < minimum * minimum
						},
					)
				blocked = RocDoomSim.any_collision(actor.spawn_pos, radius, blockers)
				if roll.byte <= 4 and !(occupied) and !(blocked) {
					fresh = RocDoomWorld.actor(actor.id, actor.kind, actor.spawn_pos, actor.spawn_angle, actor.ambush)
...
fire = |world, blockers| {
	...
	if available <= 0 or world.doom.player.health <= 0 {
		...
		shot = RocDoomWorld.hitscan(world.doom.rng, world.doom.player.weapon)
		...
		match target {
			...
			Ok(id) => {
				...
				for actor in world.doom.actors {
					if actor.id == id {
						result = RocDoomWorld.damage_actor_random(actor, shot.damage, $rng)
						$rng = result.rng
				...
				aggro = if $retarget set_aggro(world.aggro, id, RocDoomRuntime.player_sound_id, List.len($actors)) else world.aggro
...
prepare_weapon = |world, intent, firing| {
	...
	selected = match requested {
		Ok(weapon) => if RocDoomWorld.owns(world.doom.player, weapon) Ok(weapon) else Err(NotOwned)
...
fallback_weapon = |player| {
	...
	List.first(List.keep_if(candidates, |weapon| RocDoomWorld.owns(player, weapon) and weapon_has_ammo(player, weapon)))
}
...
apply_sector_hazard = |world, map, tic| {
	sector = RocDoomLevel.sector_at(map, { x: F32.to_f64(world.doom.player.sim.state.pos.x), y: F32.to_f64(world.doom.player.sim.state.pos.y) }) ?? return world
...
discover_secret = |world, map| {
	...
	sector = RocDoomLevel.sector_at(map, { x: F32.to_f64(pos.x), y: F32.to_f64(pos.y) }) ?? return world
...
damage_player_for_skill = |player, damage, skill|
	RocDoomWorld.damage_player(player, if skill == Baby damage / 2 else damage)
...
target_actor = |actors, origin, angle, spread_turns, blockers| {
	...
	facing = RocDoomSim.normalize(RocDoomSim.add(facing0, RocDoomSim.scale(perpendicular, spread_turns * 6.2831855)))
	var $target = Err(NoTarget)
	var $best_distance = 2048 * 2048
	for actor in actors {
		if actor.state.mode != Dead {
			to_actor = RocDoomSim.sub(actor.pos, origin)
			distance = RocDoomSim.length_squared(to_actor)
			along = RocDoomSim.dot(to_actor, facing)
			across = F32.abs(to_actor.x * facing.y - to_actor.y * facing.x)
			# A target counts as aimed at when the shot line clips its body, so
			# the tolerance is the actor's own radius widened by the autoaim
			# cone (`P_AimLineAttack` sweeps +/- 5.625 degrees, tan ~= 0.0985).
			tolerance = RocDoomWorld.actor_radius_for(actor.kind) + along * autoaim_slope
			if along > 0 and distance < $best_distance and across <= tolerance and RocDoomRuntime.line_of_sight(origin, actor.pos, blockers) {
...
expect {
	...
	RocDoomRuntime.line_of_sight({ x: 0, y: 0 }, { x: 64, y: 0 }, [])
		and !(RocDoomRuntime.line_of_sight({ x: 0, y: 0 }, { x: 64, y: 0 }, [wall]))
}

expect {
	path : RocDoomSim.Segment
	path = { start: { x: 0, y: 0 }, end: { x: 100, y: 0 } }
	crossing : RocDoomSim.Segment
	crossing = { start: { x: 50, y: -20 }, end: { x: 50, y: 20 } }
	grazing : RocDoomSim.Segment
	grazing = { start: { x: 40, y: 6 }, end: { x: 60, y: 6 } }
	clear : RocDoomSim.Segment
				...
				and !(actor_needs_sight({ ..base, state: { mode: Attack, remaining: 17, attacked: Bool.False } }))
					and actor_needs_sight({ ..base, state: { mode: Attack, remaining: 16, attacked: Bool.False } })
						and !(actor_needs_sight({ ..base, state: { mode: Attack, remaining: 15, attacked: Bool.True } }))
							and !(actor_needs_sight({ ..base, state: RocDoomWorld.state(Dead) }))
}

expect {
	player = RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0))
	actor = RocDoomWorld.actor(1, ZombieMan, { x: 96, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	world : RocDoomWorld.World
	world = { player, actors: [actor], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	advanced = RocDoomRuntime.advance(RocDoomRuntime.initial(world), RocDoomSim.tic_seconds, { ..RocDoomSim.neutral, fire: Bool.True }, [])
...
expect {
	player = { ..RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0)), health: 50 }
	pickup : RocDoomWorld.Pickup
	pickup = { id: 0, kind: StimpackPickup, pos: { x: 8, y: 0 }, taken: Bool.False }
	world : RocDoomWorld.World
	world = { player, actors: [], pickups: [pickup], rng: RocDoomWorld.Rng.seed(0) }
	next = RocDoomRuntime.tic(RocDoomRuntime.initial(world), Bool.False, []).world
	item = List.get(next.doom.pickups, 0) ?? pickup
	next.doom.player.health == 60 and item.taken
}

expect {
	player = RocDoomWorld.player({ x: 256, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	imp0 = RocDoomWorld.actor(1, Imp, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	imp = { ..imp0, state: { mode: Attack, remaining: 6, attacked: Bool.False } }
	doom : RocDoomWorld.World
	doom = { player, actors: [imp], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	spawned = RocDoomRuntime.tic(RocDoomRuntime.initial(doom), Bool.False, []).world
	...
	blocked = RocDoomRuntime.tic(spawned, Bool.False, [wall]).world
...
expect {
	...
	player = RocDoomWorld.player({ x: 10, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	projectile : RocDoomRuntime.Projectile
	projectile = { id: 0, owner: 1, target: RocDoomRuntime.player_sound_id, pos: { x: 0, y: 0 }, momentum: { x: 16, y: 0 }, damage: 12, remaining: 4 }
	wall : RocDoomSim.Segment
	wall = { start: { x: 4, y: -32 }, end: { x: 4, y: 32 } }
	step = advance_projectiles([projectile], player.sim.state.pos, [], [wall], RocDoomWorld.Rng.seed(0))
...
expect {
	...
	forward = RocDoomRuntime.use_forward(map, level, { x: 832, y: 512 }, RocDoomSim.Angle.from_turns(0.25), { blue: Bool.False, yellow: Bool.False, red: Bool.False })
	backward = RocDoomRuntime.use_forward(map, level, { x: 832, y: 512 }, RocDoomSim.Angle.from_turns(0.75), { blue: Bool.False, yellow: Bool.False, red: Bool.False })
	match forward {
...
expect {
	...
	doom : RocDoomWorld.World
	doom = { player, actors: [imp], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	fixture : RocDoomRuntime.Projectile
	fixture = { id: 0, owner: 99, target: RocDoomRuntime.player_sound_id, pos: { x: -1000, y: -1000 }, momentum: { x: 0, y: 0 }, damage: 1, remaining: 10 }
	full = { ..RocDoomRuntime.initial(doom), projectiles: List.repeat(fixture, RocDoomRuntime.max_projectiles) }
	result = RocDoomRuntime.tic(full, Bool.False, [])
	result.projectile_saturated and List.len(result.world.projectiles) == RocDoomRuntime.max_projectiles
}

expect {
	player = RocDoomWorld.player({ x: 100, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	a0 = RocDoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	a = { ..a0, state: { mode: Chase, remaining: 1, attacked: Bool.False } }
	b = RocDoomWorld.actor(2, ZombieMan, { x: 24, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	doom : RocDoomWorld.World
	doom = { player, actors: [a, b], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	next = RocDoomRuntime.tic(RocDoomRuntime.initial(doom), Bool.False, []).world
...
expect {
	player = RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0))
	actor = RocDoomWorld.actor(1, ZombieMan, { x: 48, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	doom : RocDoomWorld.World
	doom = { player, actors: [actor], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	advanced = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds * 8, { ..RocDoomSim.neutral, forward: 50 }, [])
	advanced.world.doom.player.sim.state.pos.x < 12
}

expect {
	player = { ..RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0)), health: 1 }
	zombie0 = RocDoomWorld.actor(1, ZombieMan, { x: 64, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	zombie = { ..zombie0, state: { mode: Attack, remaining: 16, attacked: Bool.False } }
	doom : RocDoomWorld.World
	doom = { player, actors: [zombie], pickups: [], rng: RocDoomWorld.Rng.seed(1) }
	next = RocDoomRuntime.tic(RocDoomRuntime.initial(doom), Bool.False, []).world
	next.phase == Dead and next.doom.player.health == 0
}

expect {
	start = RocDoomMap.e1m1.player_start() ?? crash "E1M1 player start missing"
	pos = { x: I64.to_f32(start.position.x), y: I64.to_f32(start.position.y) }
	normal = RocDoomWorld.actor(1, ZombieMan, pos, RocDoomSim.Angle.from_turns(0), Bool.False)
	ambush = RocDoomWorld.actor(2, ZombieMan, pos, RocDoomSim.Angle.from_turns(0), Bool.True)
	heard = heard_actor_ids(RocDoomMap.e1m1, [pos], [normal, ambush])
	List.contains(heard, normal.id) and !(List.contains(heard, ambush.id))
}

expect {
	player = RocDoomWorld.player({ x: 512, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	attacker0 = RocDoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	attacker = { ..attacker0, state: { mode: Attack, remaining: 16, attacked: Bool.False } }
	victim = RocDoomWorld.actor(2, Imp, { x: 32, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	doom : RocDoomWorld.World
	doom = { player, actors: [attacker, victim], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	world = { ..RocDoomRuntime.initial(doom), aggro: [{ hunter: attacker.id, target: victim.id }] }
	next = RocDoomRuntime.tic_hearing(world, [], []).world
...
expect {
	...
	first = RocDoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	second = RocDoomWorld.actor(2, Imp, { x: 32, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	victim0 = RocDoomWorld.actor(3, ShotgunGuy, { x: 64, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	victim = { ..victim0, target_threshold: 50 }
	result = apply_actor_hits([first, second, victim], [{ source: second.id, target: victim.id, damage: 1 }], RocDoomWorld.Rng.seed(0), [{ hunter: victim.id, target: first.id }])
...
expect {
	player = RocDoomWorld.player({ x: 512, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	owner = RocDoomWorld.actor(1, Imp, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	victim = RocDoomWorld.actor(2, ZombieMan, { x: 24, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	projectile : RocDoomRuntime.Projectile
	projectile = { id: 0, owner: owner.id, target: victim.id, pos: { x: 0, y: 0 }, momentum: { x: 16, y: 0 }, damage: 3, remaining: 4 }
	step = advance_projectiles([projectile], player.sim.state.pos, [owner, victim], [], RocDoomWorld.Rng.seed(0))
...
expect {
	player = RocDoomWorld.player({ x: 512, y: 0 }, RocDoomSim.Angle.from_turns(0.5))
	owner = RocDoomWorld.actor(1, Imp, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	interceptor = RocDoomWorld.actor(2, ZombieMan, { x: 24, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	target = RocDoomWorld.actor(3, ZombieMan, { x: 48, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	projectile : RocDoomRuntime.Projectile
	projectile = { id: 0, owner: owner.id, target: target.id, pos: { x: 0, y: 0 }, momentum: { x: 40, y: 0 }, damage: 12, remaining: 4 }
	step = advance_projectiles([projectile], player.sim.state.pos, [owner, interceptor, target], [], RocDoomWorld.Rng.seed(0))
...
expect {
	...
	map = RocDoomMap.e1m1
	level = RocDoomLevel.initial(map)
	var $state = RocDoomSim.initial({ x: 196.56944, y: 310.7271 }, RocDoomSim.Angle.from_turns(0))
	for _ in List.repeat({}, 16) {
		blockers = RocDoomRuntime.blockers_for_player(map, level, $state.pos)
		$state = RocDoomSim.tic($state, { ..RocDoomSim.neutral, forward: 50 }, blockers)
	}
	sector = RocDoomLevel.sector_at(map, { x: F32.to_f64($state.pos.x), y: F32.to_f64($state.pos.y) })
...
expect {
	...
	map = RocDoomMap.e1m1
	level = RocDoomLevel.initial(map)
	var $state = RocDoomSim.initial({ x: -640, y: 256 }, RocDoomSim.Angle.from_turns(0.5))
...
expect {
	...
	player = RocDoomWorld.player({ x: 2124, y: 958 }, RocDoomSim.Angle.from_turns(0))
	doom : RocDoomWorld.World
	doom = { player, actors: [], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	world = RocDoomRuntime.initial(doom)
	safe = apply_sector_hazard(world, RocDoomMap.e1m1, 31)
	hurt = apply_sector_hazard(world, RocDoomMap.e1m1, 32)
...
expect {
	...
	player = { ..RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0)), health: 1 }
	zombie0 = RocDoomWorld.actor(1, ZombieMan, { x: 32, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	zombie = { ..zombie0, state: { mode: Attack, remaining: 16, attacked: Bool.False } }
	medikit : RocDoomWorld.Pickup
	medikit = { id: 0, kind: MedikitPickup, pos: player.sim.state.pos, taken: Bool.False }
	doom : RocDoomWorld.World
	doom = { player, actors: [zombie], pickups: [medikit], rng: RocDoomWorld.Rng.seed(1) }
	next = RocDoomRuntime.tic(RocDoomRuntime.initial(doom), Bool.False, []).world
...
expect {
	...
	player = RocDoomWorld.player({ x: 256, y: 0 }, RocDoomSim.Angle.from_turns(0))
	base = RocDoomWorld.actor(1, Demon, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	actor = { ..base, state: RocDoomWorld.state_for(Demon, Chase) }
	doom : RocDoomWorld.World
	doom = { player, actors: [actor], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	medium = RocDoomRuntime.tic(RocDoomRuntime.initial_for_skill(doom, Medium), Bool.False, []).world
	nightmare0 = RocDoomRuntime.initial_for_skill(doom, Nightmare)
	nightmare = RocDoomRuntime.tic(nightmare0, Bool.False, []).world
...
expect {
	...
	doom : RocDoomWorld.World
	doom = { player, actors: [attacker], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	medium = RocDoomRuntime.tic(RocDoomRuntime.initial_for_skill(doom, Medium), Bool.False, []).world
	nightmare = RocDoomRuntime.tic(RocDoomRuntime.initial_for_skill(doom, Nightmare), Bool.False, []).world
	...
	RocDoomSim.length(medium_projectile.momentum) == 10 and RocDoomSim.length(nightmare_projectile.momentum) == 20
}
...
expect {
	...
	doom : RocDoomWorld.World
	doom = { player, actors: [], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	advanced = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds, { ..RocDoomSim.neutral, fire: Bool.True }, [])
...
expect {
	...
	doom : RocDoomWorld.World
	doom = { player: owned, actors: [], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	selected = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds, { ..RocDoomSim.neutral, weapon_slot: SelectSlot(4) }, []).world
	ignored = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds, { ..RocDoomSim.neutral, weapon_slot: SelectSlot(6) }, []).world
...
expect {
	...
	map = RocDoomMap.e1m1
	closed_level = RocDoomLevel.initial(map)
	player = RocDoomWorld.player({ x: 832, y: 520 }, RocDoomSim.Angle.from_turns(0.25))
	actor = RocDoomWorld.actor(1, ZombieMan, { x: 832, y: 568 }, RocDoomSim.Angle.from_turns(0.75), Bool.False)
	doom : RocDoomWorld.World
	doom = { player, actors: [actor], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	closed = RocDoomRuntime.tic_hearing_in_map(RocDoomRuntime.initial(doom), [], [], map, closed_level).world
	activated = RocDoomLevel.use_line(map, closed_level, 55, { blue: Bool.False, yellow: Bool.False, red: Bool.False })
	var $open_level = match activated {
		Activated(value) => value
		_ => crash "E1M1 door 55 did not activate"
	}
	for _ in List.repeat({}, 80) {
		$open_level = RocDoomLevel.tick($open_level)
	}
	opened = RocDoomRuntime.tic_hearing_in_map(RocDoomRuntime.initial(doom), [], [], map, $open_level).world
...
expect {
	...
	map = RocDoomMap.e1m1
	level = RocDoomLevel.initial(map)
	player = RocDoomWorld.player({ x: 832, y: 512 }, RocDoomSim.Angle.from_turns(0.25))
	base = RocDoomWorld.actor(1, ZombieMan, { x: 832, y: 568 }, RocDoomSim.Angle.from_turns(0.75), Bool.False)
	actor = { ..base, reaction_time: 0, state: { mode: Chase, remaining: 1, attacked: Bool.False } }
	doom : RocDoomWorld.World
	doom = { player, actors: [actor], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	advanced = RocDoomRuntime.advance_in_map(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds, RocDoomSim.neutral, [], map, level)
...
expect {
	player = RocDoomWorld.player({ x: 508, y: 800 }, RocDoomSim.Angle.from_turns(0))
	doom : RocDoomWorld.World
	doom = { player, actors: [], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	initial = RocDoomRuntime.initial(doom)
	first = discover_secret(initial, RocDoomMap.e1m1)
	second = discover_secret(first, RocDoomMap.e1m1)
...
expect {
	player = RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0))
	doom : RocDoomWorld.World
	doom = { player, actors: [], pickups: [], rng: RocDoomWorld.Rng.seed(7) }
	command = { ..RocDoomSim.neutral, fire: Bool.True }
	whole = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds * 8, command, []).world
	first = RocDoomRuntime.advance(RocDoomRuntime.initial(doom), RocDoomSim.tic_seconds * 3, command, []).world
	partitioned = RocDoomRuntime.advance(first, RocDoomSim.tic_seconds * 5, command, []).world
...
expect {
	...
	player = RocDoomWorld.player({ x: 256, y: 0 }, RocDoomSim.Angle.from_turns(0))
	alive = RocDoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	dead = RocDoomWorld.damage_actor(alive, 100)
	doom : RocDoomWorld.World
	doom = { player, actors: [dead], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	first = resolve_deaths([alive], RocDoomRuntime.initial(doom), [])
...
expect {
	...
	player0 = RocDoomWorld.player({ x: 100, y: 0 }, RocDoomSim.Angle.from_turns(0))
	player = { ..player0, health: 200 }
	first_alive = RocDoomWorld.actor(1, Barrel, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	second_alive = RocDoomWorld.actor(2, Barrel, { x: 50, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	first_dead0 = RocDoomWorld.damage_actor(first_alive, 100)
	...
	doom : RocDoomWorld.World
	doom = { player, actors: [first_action, second_alive], pickups: [], rng: RocDoomWorld.Rng.seed(0) }
	first_result = resolve_deaths([first_before, second_alive], RocDoomRuntime.initial(doom), [])
...
expect {
	...
	player0 = RocDoomWorld.player({ x: 256, y: 0 }, RocDoomSim.Angle.from_turns(0))
	player = { ..player0, sim: { ..player0.sim, state: { ..player0.sim.state, tic: 32 } } }
	spawned = RocDoomWorld.actor(1, ZombieMan, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0.25), Bool.True)
	moved = { ..spawned, pos: { x: 100, y: 0 } }
	dead0 = RocDoomWorld.damage_actor(moved, 100)
	dead = { ..dead0, dead_tics: 12 * RocDoomSim.tic_rate }
	doom : RocDoomWorld.World
	doom = { player, actors: [dead], pickups: [], rng: RocDoomWorld.Rng.seed(65) }
	result = nightmare_respawns(RocDoomRuntime.initial_for_skill(doom, Nightmare), [])
...
expect {
	...
	player0 = RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0))
	player = { ..player0, sim: { ..player0.sim, state: { ..player0.sim.state, tic: 32 } } }
	alive = RocDoomWorld.actor(1, Imp, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	dead0 = RocDoomWorld.damage_actor(alive, 1000)
	dead = { ..dead0, dead_tics: 12 * RocDoomSim.tic_rate }
	doom : RocDoomWorld.World
	doom = { player, actors: [dead], pickups: [], rng: RocDoomWorld.Rng.seed(65) }
	result = nightmare_respawns(RocDoomRuntime.initial_for_skill(doom, Nightmare), [])
...
expect {
	# A point-blank demon overlapping the player used to be un-hittable: the
	# aim corridor was a fixed 24 units, narrower than the demon's own radius.
	demon = RocDoomWorld.actor(1, Demon, { x: 40, y: 28 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	target_actor([demon], { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), 0, []) == Ok(demon.id)
}

expect {
	# Distant targets stay hittable within the autoaim cone rather than a
	# corridor that narrows to a fraction of a degree at range.
	imp = RocDoomWorld.actor(1, Imp, { x: 800, y: 60 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	far = RocDoomWorld.actor(2, Imp, { x: 800, y: 400 }, RocDoomSim.Angle.from_turns(0), Bool.False)
	target_actor([imp], { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), 0, []) == Ok(imp.id)
		and target_actor([far], { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), 0, []) == Err(NoTarget)
}
