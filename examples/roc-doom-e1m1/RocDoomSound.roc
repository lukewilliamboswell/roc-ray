...
RocDoomSound := [].{
	heard_sectors : RocDoomMap.Map, { x : F32, y : F32 } -> List(U64)
	heard_sectors = |map, source| {
		start = RocDoomLevel.sector_at(map, { x: F32.to_f64(source.x), y: F32.to_f64(source.y) }) ?? return []
		flood(map.raw(), [{ sector: start, blocks: 0.U8 }], [], [])
	}

	can_hear : RocDoomMap.Map, { x : F32, y : F32 }, { x : F32, y : F32 } -> Bool
	can_hear = |map, source, listener| {
		sector = RocDoomLevel.sector_at(map, { x: F32.to_f64(listener.x), y: F32.to_f64(listener.y) }) ?? return Bool.False
		...
		match line_sectors(raw, line) {
			...
			Ok(pair) => {
				...
				match neighbor {
					Err(_) => {}
					Ok(sector) => {
						crosses_block = RocDoomMap.line_flags(line.flags).blocks_sound
...
expect {
	...
	heard = RocDoomSound.heard_sectors(RocDoomMap.e1m1, pos)
	List.len(heard) > 1 and RocDoomSound.can_hear(RocDoomMap.e1m1, pos, pos)
}
