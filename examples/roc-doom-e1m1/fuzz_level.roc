...
## the caller's list fields when compiled with `roc build`. RocDoomLevel builds
...
tick = |state| RocDoomLevel.tick(copy_state(state))

use_line = |map, state, line, keys| RocDoomLevel.use_line(map, copy_state(state), line, keys)

cross_line = |map, state, line| RocDoomLevel.cross_line(map, copy_state(state), line)
...
check_tick = |map, before, after, step_desc| {
	for entry in List.map_with_index(after.heights, |h, index| { h, index }) {
		...
		if after.tic % 7 == 0 {
			light = RocDoomLevel.light_for(map, after, entry.index) ?? crash "light_for range"
...
direct_changed = |map, before, after| {
	...
	for entry in List.map_with_index(after.heights, |h, index| { h, index }) {
		prev = List.get(before.heights, entry.index) ?? crash "height list shrank"
		lb = RocDoomLevel.light_for(map, before, entry.index) ?? crash "light"
		la = RocDoomLevel.light_for(map, after, entry.index) ?? crash "light"
...
check_render_changed = |map, state, step_desc| {
	if RocDoomLevel.render_changed(map, state, state) {
		crash "PROPERTY: render_changed(s,s) must be False at ${step_desc}"
	}
	after = tick(state)
	claimed = RocDoomLevel.render_changed(map, state, after)
...
run_stream = |map, specials, input, checked| {
	...
	var $state = RocDoomLevel.initial(map)
	var $total = 0
	var $step = 0
