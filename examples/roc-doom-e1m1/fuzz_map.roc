...
## Property fuzz target for RocDoomMap validation + RocDoomLevel spatial queries.
## Core property: anything `RocDoomMap.validate` accepts must be safe to query.
...
## RocDoomLevel.dynamic_sectors returns garbage indices for some maps while the
...
child_of : U8, U64, U64, U64, U8 -> RocDoomMap.Child
...
polygon_points : PolySeed -> List(RocDoomMap.SurfacePoint)
polygon_points = |seed| {
...
build_raw : Input -> RocDoomMap.Raw
build_raw = |input| {
...
nodes_acyclic : RocDoomMap.Raw -> Bool
nodes_acyclic = |raw| {
	List.all(
...
has_zero_length_line : RocDoomMap.Raw -> Bool
...
sector_of_side : RocDoomMap.Raw, Try(U64, [Null]) -> Try(U64, [None])
...
sector_has_two_sided_line : RocDoomMap.Raw, U64 -> Bool
...
query_point : RocDoomMap.Raw, QuerySeed -> RocDoomLevel.Point
...
second_point : QuerySeed -> RocDoomLevel.Point
second_point = |q| { x: U64.to_f64(q.py) * 0.5 - 40, y: U64.to_f64(q.px) * 0.5 - 40 }

keys_of : U8 -> RocDoomLevel.Keys
keys_of = |k| { blue: k % 2 == 1, yellow: k % 4 >= 2, red: k % 8 >= 4 }

check_spans : RocDoomMap.Raw, List(RocDoomMap.WallSpan), Str -> {}
...
tick_n : RocDoomLevel.State, U8 -> RocDoomLevel.State
tick_n = |state, n| if n == 0 state else tick_n(RocDoomLevel.tick(state), n - 1)

exercise_state : RocDoomMap.Map, RocDoomMap.Raw, RocDoomLevel.State, RocDoomLevel.State, QuerySeed, Str -> {}
exercise_state = |map, raw, base, state, q, label| {
	...
	_ = RocDoomLevel.render_changed(map, base, ticked)
	_ = map.wall_spans_at(ticked.heights)
	for sector in indices(nsec) {
		_ = RocDoomLevel.light_for(map, ticked, sector)
		_ = RocDoomLevel.heights_for(ticked, sector)
		for segment in RocDoomLevel.collision_segments(map, ticked, sector) {
	...
	match RocDoomMap.validate(raw) {
		...
		Ok(map) => {
			if !nodes_acyclic(raw) {
				...
				match RocDoomLevel.sector_at(map, point) {
					Ok(sector) => ensure(sector < nsec, "PROPERTY: sector_at returned out-of-range sector")
					Err(OutsideMap) => {}
				}
				for line in RocDoomLevel.crossed_lines(map, point, second_point(q)) {
				...
				dyn = RocDoomLevel.dynamic_sectors(map)
				...
				_ = RocDoomLevel.portal(map, state, q.line, q.sector)
				_ = RocDoomLevel.collision_candidate_count(state, q.sector)

				# Activation vocabulary over every linedef
				for line in indices(nl) {
					for sector in indices(nsec) {
						_ = RocDoomLevel.portal(map, state, line, sector)
					}
					match RocDoomLevel.use_line(map, state, line, keys_of(q.keys)) {
						Activated(next) => exercise_state(map, raw, state, next, q, "use_line")
						_ => {}
					}
					match RocDoomLevel.cross_line(map, state, line) {
