...
## Property fuzzing for the pure `RocDoomWorld` inventory/combat layer.
##
## Properties:
...
pickup_kind : U8 -> RocDoomWorld.ThingKind
pickup_kind = |index|
	match index {
		0 => ShotgunPickup
		1 => ClipPickup
...
actor_kind : U8 -> RocDoomWorld.ThingKind
actor_kind = |index|
	match index {
		0 => ZombieMan
		1 => ShotgunGuy
...
vanilla_accepts : RocDoomWorld.Player, RocDoomWorld.ThingKind -> Bool
...
check_player : RocDoomWorld.Player, Str -> {}
check_player = |p, ctx| {
	...
	if !(RocDoomWorld.owns(p, p.weapon)) {
...
same_player : RocDoomWorld.Player, RocDoomWorld.Player -> Bool
same_player = |a, b|
	a.health == b.health
		and a.armor == b.armor
...
same_vec : RocDoomSim.Vec2, RocDoomSim.Vec2 -> Bool
same_vec = |a, b| F32.to_bits(a.x) == F32.to_bits(b.x) and F32.to_bits(a.y) == F32.to_bits(b.y)

same_turn : RocDoomWorld.ActorTic, RocDoomWorld.ActorTic -> Bool
same_turn = |a, b|
...
pickup_at : RocDoomWorld.ThingKind, U64 -> RocDoomWorld.Pickup
pickup_at = |kind, id| { id, kind, pos: { x: 0, y: 0 }, taken: Bool.False }

test_pickups : Input -> {}
test_pickups = |input| {
	base = RocDoomWorld.player({ x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0))
	start = if input.backpack_first == 1 RocDoomWorld.collect(base, pickup_at(BackpackPickup, 999)).player else base
	var $player = start
	var $index = 0
	...
	for byte in input.pickups {
		...
		before = $player
		result = RocDoomWorld.collect(before, pickup_at(kind, $index))
		after = result.player
		...
		# These kinds are consumed without necessarily changing the player
		# (vanilla-consistent), so "collected iff changed" does not apply to them.
		always_consumed = is_weapon or kind == BerserkPickup or kind == LightAmpPickup or kind == SoulSpherePickup or kind == HealthBonusPickup or (before.armor >= 200 and kind == ArmorBonusPickup)
		...
		if result.collected != changed and !(always_consumed) {
		...
		if result.collected != vanilla_accepts(before, kind) {
		...
		again = RocDoomWorld.collect(after, result.pickup)
		...
		sk = RocDoomWorld.collect_for_skill(before, pickup_at(kind, $index), skill(input.skill))
	...
	for raw in input.damages {
		...
		damaged = RocDoomWorld.damage_player($player, amount)
...
test_firing = |input| {
	...
	actor = RocDoomWorld.actor(0, actor_kind(input.actor_kind), { x: 64, y: 0 }, RocDoomSim.Angle.from_turns(0.5), Bool.False)
	world : RocDoomWorld.World
	world = { player: player0, actors: [actor, { ..actor, id: 1 }], pickups: [], rng: RocDoomWorld.Rng.seed(input.fire_seed) }
	command = { ..RocDoomSim.neutral, fire: Bool.True }
	var $world = world
	var $count = 0
	while $count < U8.to_u64(input.fire_ticks) {
		before = $world
		$world = RocDoomWorld.world_tic(before, command)
		p = $world.player
...
test_actor = |input| {
	...
	actor = RocDoomWorld.actor(7, kind, { x: 0, y: 0 }, RocDoomSim.Angle.from_turns(0), input.actor_ambush == 1)
	...
	var $turn = { actor, rng: RocDoomWorld.Rng.seed(input.seed), player_damage: 0, attack_kind: NoAttack }
	var $turn2 = { actor, rng: RocDoomWorld.Rng.seed(input.seed), player_damage: 0, attack_kind: NoAttack }
	var $count = 0
	var $damage_index = 0
	while $count < U8.to_u64(input.tics) {
		prev = $turn
		$turn = RocDoomWorld.tick_actor_with(prev.actor, facts, prev.rng)
		$turn2 = RocDoomWorld.tick_actor_with($turn2.actor, facts, $turn2.rng)
		...
		action_tic = prev.actor.state.mode == Attack and !(prev.actor.state.attacked) and prev.actor.state.remaining == RocDoomWorld.attack_action_remaining(prev.actor.kind)
		...
		if !(facts.has_sight) and !(facts.heard_sound) and prev.actor.state.mode != Look {
			plain = RocDoomWorld.tick_actor(prev.actor)
		...
		if $count % 7 == 6 {
			match List.get(input.damages, $damage_index) {
				...
				Ok(raw) => {
					amount = U64.to_i64_wrap(raw)
					plain = RocDoomWorld.damage_actor(a, amount)
					rolled = RocDoomWorld.damage_actor_random(a, amount, $turn.rng)
