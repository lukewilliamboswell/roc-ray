...
Model : { initial : RocDoomSim.State, initial_ok : Bool, right_ok : Bool }
...
init! = App.init(
	...
	|_startup| {
		...
		player = RocDoomWorld.player(pos, angle)
		sector = RocDoomLevel.sector_at(map, { x: I64.to_f64(start.position.x), y: I64.to_f64(start.position.y) }) ?? crash "E1M1 start outside map"
		heights = RocDoomLevel.heights_for(RocDoomLevel.initial(map), sector) ?? crash "E1M1 start sector missing"
...
update! = |model, input| {
	...
	if cycle == 0 {
		...
		right_state = RocDoomSim.tic(model.initial, { ..RocDoomSim.neutral, side: RocDoomControls.side(Bool.False, devices.key_down(KeyD)) }, [])
		right_delta = RocDoomSim.sub(right_state.pos, model.initial.pos)
		right_ok = devices.key_pressed(KeyD)
			and devices.key_down(KeyD)
				and RocDoomSim.dot(right_delta, RocDoomControls.visual_right(model.initial.angle)) > 0
		Keys.set_source!(Keys.holding([KeyA]))
		Mouse.set_source!(Mouse.virtual_at({ x: 120, y: 80 }))
		Ok({ ..model, right_ok })
	} else if cycle == 2 {
		left_state = RocDoomSim.tic(model.initial, { ..RocDoomSim.neutral, side: RocDoomControls.side(devices.key_down(KeyA), Bool.False) }, [])
		left_delta = RocDoomSim.sub(left_state.pos, model.initial.pos)
		mouse_delta = devices.mouse.delta().x
		turned = model.initial.angle.add(RocDoomControls.turn(mouse_delta)).forward()
		left_ok = devices.key_released(KeyD)
			and devices.key_pressed(KeyA)
				and devices.key_down(KeyA)
					and RocDoomSim.dot(left_delta, RocDoomControls.visual_right(model.initial.angle)) < 0
		turn_ok = mouse_delta > 0 and RocDoomSim.dot(turned, RocDoomControls.visual_right(model.initial.angle)) > 0
