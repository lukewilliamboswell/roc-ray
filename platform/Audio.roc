...
Audio := [].{
	...
	Sound :: { resource : Host.AudioSound }.{
		...
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		stop! : Sound => {}
		stop! = |sound| Host.audio_stop_sound!(sound.resource)

		## Pause playback at the current position.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		pause! : Sound => {}
		pause! = |sound| Host.audio_pause_sound!(sound.resource)

		## Resume a paused sound.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		resume! : Sound => {}
		resume! = |sound| Host.audio_resume_sound!(sound.resource)

		## Whether this sound is currently playing.
		##
		## Legal in any callback, `render!` included.
		is_playing! : Sound => Bool
		is_playing! = |sound| Host.audio_is_sound_playing!(sound.resource)
	...
	Playback := { sound : Sound, volume : F32, pitch : F32, pan : F32 }.{
		...
		play! = |Playback.(settings)| {
			Host.audio_set_sound_volume!(settings.sound.resource, settings.volume)
			Host.audio_set_sound_pitch!(settings.sound.resource, settings.pitch)
			Host.audio_set_sound_pan!(settings.sound.resource, settings.pan)
			Host.audio_play_sound!(settings.sound.resource)
		}
	}
	...
	Music :: { resource : Host.AudioMusic }.{
		...
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		play! : Music => {}
		play! = |music| Host.audio_play_music!(music.resource)

		## Stop playback and rewind.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		stop! : Music => {}
		stop! = |music| Host.audio_stop_music!(music.resource)

		## Pause at the current position.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		pause! : Music => {}
		pause! = |music| Host.audio_pause_music!(music.resource)

		## Resume paused playback.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		resume! : Music => {}
		resume! = |music| Host.audio_resume_music!(music.resource)

		## Set stream volume, clamped to 0 through 1.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		set_volume! : Music, F32 => {}
		set_volume! = |music, volume| Host.audio_set_music_volume!(music.resource, volume)

		## Set stream pitch multiplier.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		set_pitch! : Music, F32 => {}
		set_pitch! = |music, pitch| Host.audio_set_music_pitch!(music.resource, pitch)

		## Set stereo pan, clamped to -1 through 1.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		set_pan! : Music, F32 => {}
		set_pan! = |music, pan| Host.audio_set_music_pan!(music.resource, pan)

		## Enable or disable automatic looping.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		set_looping! : Music, Bool => {}
		set_looping! = |music, looping| Host.audio_set_music_looping!(music.resource, looping)

		## Whether this stream is currently playing.
		##
		## Legal in any callback, `render!` included.
		is_playing! : Music => Bool
		is_playing! = |music| Host.audio_is_music_playing!(music.resource)

		## Seek to seconds from the start. Negative values are clamped to zero.
		##
		## Legal in `init!`, `update!`, and tasks; refused in `render!`.
		seek! : Music, F32 => {}
		seek! = |music, seconds| Host.audio_seek_music!(music.resource, seconds)

		## Total stream length in seconds, or zero for an invalid resource.
		##
		## Legal in any callback, `render!` included.
		length! : Music => F32
		length! = |music| Host.audio_music_length!(music.resource)

		## Current playback position in seconds.
		##
		## Legal in any callback, `render!` included.
		time_played! : Music => F32
		time_played! = |music| Host.audio_music_time_played!(music.resource)
	...
	set_master_volume! = |volume| Host.audio_set_master_volume!(volume)
...
raw_config : Audio.GenSound -> Host.AudioGenSound
raw_config = |cfg| {
