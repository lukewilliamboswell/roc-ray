...
Keys := [].{
	...
	set_source! = |source|
		match source {
			Hardware => HostABI.capture_set_virtual_keys!({ active: Bool.False, keys: [] })
			Virtual(keys) => HostABI.capture_set_virtual_keys!({ active: Bool.True, keys: List.map(keys, key_code) })
		}
	...
	## ```
	set_text! : List(U32) => {}
	set_text! = |text| HostABI.capture_set_virtual_text!(text)
}
