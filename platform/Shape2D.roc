...
import Raylib exposing [Color]
import Effect

Shape2D := [
    DrawText { text : Str, posX : F32, posY : F32, size : I32, color : Color },
    DrawRectangle { posX : F32, posY : F32, width : F32, height : F32, color : Color },
    DrawRectangleGradientV { posX : F32, posY : F32, width : F32, height : F32, top : Color, bottom : Color },
    DrawCircle { centerX : F32, centerY : F32, radius : F32, color : Color },
    DrawCircleGradient { centerX : F32, centerY : F32, radius : F32, inner : Color, outer : Color },
]
    implements [Drawable { draw: drawShape }]

# Draw a color-filled rectangle
rect : { posX : F32, posY : F32, width : F32, height : F32, color : Color } -> Shape2D
rect = \config -> DrawRectangle config |> @Shape2D

# Draw a vertical-gradient-filled rectangle
rectGradientV : { posX : F32, posY : F32, width : F32, height : F32, top : Color, bottom : Color } -> Shape2D
rectGradientV = \config -> DrawRectangleGradientV config |> @Shape2D

# Draw text (using default font)
text : { text : Str, posX : F32, posY : F32, size : I32, color : Color } -> Shape2D
text = \config -> DrawText config |> @Shape2D

# Draw a color-filled circle
circle : { centerX : F32, centerY : F32, radius : F32, color : Color } -> Shape2D
circle = \config -> DrawCircle config |> @Shape2D

# Draw a gradient-filled circle
circleGradient : { centerX : F32, centerY : F32, radius : F32, inner : Color, outer : Color } -> Shape2D
