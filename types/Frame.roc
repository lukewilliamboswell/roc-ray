...
frame.Drawable(font, texture) :
	where [
		...
		frame.texture! : frame, { texture : texture, source : Math.Rect, dest : Math.Rect, origin : Math.Vec2, rotation : F32, tint : Color.Rgba } => {},
...
Frame := [].{
	...
	TextureDraw(texture) : { texture : texture, source : Math.Rect, dest : Math.Rect, origin : Math.Vec2, rotation : F32, tint : Color.Rgba }
}
